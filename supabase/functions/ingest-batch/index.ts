...
serve(async (req) => {
  ...
  try {
    ...
    if (!batch || batch.length === 0) {
      ...
      return new Response(
        JSON.stringify({ done: true, chunks: src?.chunk_count || 0 }),
    ...
    for (const item of batch) {
      try {
        if (item.content) {
          // PDF path — content already extracted, treat as single chunk
          allChunks.push({
            content: item.content,
            section_title: item.content.split("\n")[0].replace(/^#+\s*/, "").trim().slice(0, 200),
            section_path: item.content.split("\n")[0].replace(/^#+\s*/, "").trim().slice(0, 300),
            source_url: item.url,
          });
        } else {
          // Web scraping path
    ...
    if (allChunks.length > 0) {
      try {
        ...
        console.error("Embed/store error:", embedErr);
        // Still mark batch done so we don't retry forever
        await supabase.rpc("increment_processed_urls", {
          p_source_id: source_id,
          p_count: batch.length,
          p_chunks: 0,
    ...
    return new Response(
      JSON.stringify({ done: false, processed: batch.length, remaining: remaining || 0, chunks_added: allChunks.length }),
      { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (e) {
